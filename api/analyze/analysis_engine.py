...
"""
    ...
    table_lines = []
    table_lines.append("RANK | EXPIRATION | STRIKES | NET COST | NET VEGA | NET VOLGA | EFFICIENCY | SCORE")
    ...
    for i, row in results.head(5).iterrows():
        ...
        table_lines.append(f"{i+1:4} | {row['expiration']:10} | {strikes:15} | {cost_txt:9} | {row['net_vega']:8.3f} | {row['net_volga']:9.3f} | {row['efficiency']:9.1%} | {row['total_score']:.3f}")
